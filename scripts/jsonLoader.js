...
function newsLoader(datos) {
  ...
  for (i = 0; i < vectorObj.noticia.length; i++) {
    ...
    spanCat.appendChild(
      document.createTextNode(vectorObj.noticia[i].categoria)
    );

    //CREAMOS EL SPAN PARA AUTOR
    let spanNombre = document.createElement("span");
    spanNombre.appendChild(document.createTextNode("Por: "));
    spanNombre.appendChild(
      document.createTextNode(vectorObj.noticia[i].nombre)
    );
    spanNombre.appendChild(document.createTextNode(" "));
    spanNombre.appendChild(
      document.createTextNode(vectorObj.noticia[i].apellido)
    );
    ...
    titleLink.appendChild(
      document.createTextNode(vectorObj.noticia[i].titular)
    );
    ...
    subtitle.appendChild(
      document.createTextNode(vectorObj.noticia[i].subtitulo)
    );
