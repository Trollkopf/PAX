(function ($) {
  $.fn.visualLightbox = function (options) {
    ...
    if (options.animate) {
      ...
      var resizeDuration = (11 - options.resizeSpeed) * 0.8;
    } else {
    ...
    updateImageList();
    var objBody =
      container.length && container.get(0) != document
        ? container.get(0)
    ...
    var start = (this.start = function (imageLink) {
      ...
      if (options.startZoom) {
        ...
        $$("overlay")
          ...
          .show()
          .fadeTo(overlayDuration * 1000, options.overlayOpacity);
        $$("lightbox").css({
          left: 0,
          width: "100%",
        });
    ...
    function adjustImageSize(recall, noImage) {
      ...
      var imgTop =
        getPageScroll().y +
        (getPageSize().winHeight -
          (imgHeight + descriptionHeight + options.borderSize * 2)) /
          2;
        ...
        if (!noImage) {
          if (options.startZoom) {
            ...
            lightboxImage.css({
              ...
              left:
                (100 - zoomF * (1 > resizeFactor ? 1 : resizeFactor)) / 2 + "%",
              top:
                (100 - zoomF * (1 > resizeFactor ? 1 / resizeFactor : 1)) / 2 +
                "%",
            });
          }
          if (options.startZoom) {
    ...
    function updateDetails() {
      ...
      if (imageArray.length > 1) {
        var num_display =
          options.strings.numDisplayPrefix +
          " " +
          eval(activeImage + 1) +
          " " +
        ...
        if (options.showGroupName && groupName) {
          num_display +=
            " " + options.strings.numDisplaySeparator + " " + groupName;
        }
    ...
    function updateNav() {
      ...
      descriptionHeight =
        descriptionHeight * (1 - d) + $$("imageDataContainer").height() * d;
      if (imageArray.length > 1) {
        $$("hoverNav").show();
        if (enableSlideshow) {
          if (playSlides) {
    ...
    function showNext() {
      if (imageArray.length > 1) {
        ...
        if (
          !options.loop &&
          ((activeImage == imageArray.length - 1 && startImage == 0) ||
            activeImage + 1 == startImage)
        ) {
          end();
          return;
        }
    ...
    function preloadNeighborImages() {
      var nextImageID =
        imageArray.length - 1 == activeImage ? 0 : activeImage + 1;
      new Image().src = imageArray[nextImageID].link;
      var prevImageID =
        activeImage == 0 ? imageArray.length - 1 : activeImage - 1;
